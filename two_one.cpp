...
class complex_binary_alter{
    point a, n, r, b, points_list[16];
    short a_bits[4], b_bits[4];
public:
    void create_r(){
        unsigned seed = chrono::steady_clock::now().time_since_epoch().count();
        default_random_engine e (seed);
        normal_distribution<double> dist_gauss(0,sqrt(4));

        r = point(a.get_x() + dist_gauss(e), a.get_y() + dist_gauss(e));
        cout<<"point r: ";
        r.display();
    }
    ...
    short demap_b(){
        ...
        cout<< "binary sequence of b: ";
        for(int i=0; i<4; i++){
            cout<< b_bits[i];
        }
        cout<< endl;

        cout<< "bit differences: " << count_bit_differences();
    ...
    void create_points_list(){
        ...
        for(int i=1; i<4; i+=2){
            for(int j=1; j<4; j+=2){
                for(int k=0; k<4; k++){
                    this->points_list[index] = point(i*sign_x[k], j*sign_y[k]);
                    index++;
                }
            }
        }
...
cout<< "total bit differences: " << sum;
return 0;
}
